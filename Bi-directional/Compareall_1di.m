% Comparision of all new algorithm
% 7cores 
    ...
    for x=1:5000%request(x)
        ...
        if y==2
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
                [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths_25m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
             %[resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di_25m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
            for i=1:length(shortestpaths{k_index})
                pathmemory(x,i)=shortestpaths{k_index}(i);
            end
        end
         ...
         if y==3
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
               [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths_200m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
             %[resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di_100m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
               for i=1:length(shortestpaths{k_index})
                pathmemory(x,i)=shortestpaths{k_index}(i);
            end
         end
         ...
         if y==4
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
                [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths_250m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
             %[resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di_250m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
               for i=1:length(shortestpaths{k_index})
                pathmemory(x,i)=shortestpaths{k_index}(i);
            end
         end
         ...
         if y==5
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
                [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths_700m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
            % [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di_1000m(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
...
subplot(5,1,1);
     ...
     if y==1
         ...
         bar(p);
       % bar(XTcondition);
       title('Blocking percentage between resource congestion and crosstalk of 2di priority core switch spectrum split start2');
       set(gca,'XTickLabel',{'Crosstalk','Resource congestion'});
       hold on;
    ...
    if y==2
        ...
        bar(p);
       % bar(XTcondition);
       title('Blocking percentage between resource congestion and crosstalk of 2di priority core switch spectrum split start1');
       set(gca,'XTickLabel',{'Crosstalk','Resource congestion'});
        ylabel('Percentage of blocking');
         grid on;
    end
    ...
    if y==3
        ...
        bar(p);
       % bar(XTcondition);
       title('Blocking percentage between resource congestion and crosstalk of 2di priority core switch start1');
       set(gca,'XTickLabel',{'Crosstalk','Resource congestion'});
        ylabel('Percentage of blocking');
         grid on;
    end
    ...
    if y==4
        ...
        bar(p);
       % bar(XTcondition);
       title('Blocking percentage between resource congestion and crosstalk of 1di priority core switch start1');
       set(gca,'XTickLabel',{'Crosstalk','Resource congestion'});
        ylabel('Percentage of blocking');
         grid on;
    end
    ...
    if y==5
       ...
       bar(p);
       % bar(XTcondition);
       title('Blocking percentage between resource congestion and crosstalk of 2di priority core switch spectrum hard split start2');
       set(gca,'XTickLabel',{'Crosstalk','Resource congestion'});
        ylabel('Percentage of blocking');
         grid on;
    end
    ...
    title('resource of the link from node 1 to node 6 ');
