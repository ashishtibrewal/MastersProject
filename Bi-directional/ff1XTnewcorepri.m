function [resource,blocking,XTtotal,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock]=ff1XTnewcorepri(BW,path,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XTtotal)
XTblock=0;
EXTblock=0;
blocking=0;
n=length(path);
        ...
        if sum==(n-1)*BW&&XT2<=-24
            ...
            if length(ReqNums)~=1
                ...
                if upblocking~=0
                    ...
                    [resource,LXPR,NumSlots,linkblock]=ff1XTconvertDB(path,resource,x,LXPR,NumSlots,linkblock);
                    XTtotal(x)=0;
                    blocking=1;
                    out=1;
                    break
