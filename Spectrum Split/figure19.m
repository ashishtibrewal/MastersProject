...
[ coreseq3  coreseq4] = prioritymap7core_start1(  );
...
%           
        ...
        if y==1
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
              [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di_ss_(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT,coreseq3,coreseq4);
            for i=1:length(shortestpaths{k_index})
                pathmemory(x,i)=shortestpaths{k_index}(i);
            end
        end
         ...
         if y==2
            k_paths=1;
             ...
             [shortestpaths,kdistances]=kShortestPath(link,sta(x),dst(x),k_paths);
             [resource,blocking,XT,LXPR,NumSlots,ReqNums,XTblock,EXTblock,linkblock,k_index,Bcauses(x,:)]=ff1XTnewcoreswitchKpaths2di(BW(x),shortestpaths,link,resource,x,LXPR,NumSlots,linkblock,pathmemory,XT);
      ...
      % in whole links of each core.
          resource1(resource1<0)=0;
            ...
            if y==1;
              ...
              if x==400;
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.11 network utilization)','FontSize', 12);
              end
                subplot(6,1,2);
                if x==800;
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.24 network utilization)','FontSize', 12);
                end
                subplot(6,1,3);
                if x==1100;
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.33 network utilization)','FontSize', 12);
                end
                subplot(6,1,4);
                if x==1300
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.38 network utilization)','FontSize', 12);
                end
                subplot(6,1,5);
                if x==1500
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.44 network utilization)','FontSize', 12);
                end
                subplot(6,1,6);
                if x==2000
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.55 network utilization)','FontSize', 12);
                end
            end  
             ...
             if y==2;
              ...
              if x==400;
                ...
                if x==1500
                imshow(resource(:,:,1,6));
                title('resource of the link from node 1 to node 6 (0.44 etwork utilization)','FontSize', 12);
                end
                subplot(6,1,6);
                if x==2000
                imshow(resource(:,:,1,6));
