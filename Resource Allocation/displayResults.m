...
nResourceAllocationFailed_IT = size(find(requestDB(:,7) == 0), 1);
...
nResourceAllocationFailed_Network= size(find(requestDB(:,8) == 0), 1);
...
nRequestDropped = size(find(requestDB(:,9) == 0), 1);
