function [shortestPaths, distance] = kShortestPath(link,source, destination, k_paths)
...
if source > size(link,1) || destination > size(link,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [cost path] = dijkstra(link,source, destination);
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                for i = 1: length(sub_P)-1
                    cost_sub_P = cost_sub_P + link(sub_P(i),sub_P(i+1));
                end

                %call dijkstra between deviation vertex to destination node    
                [c dev_p] = dijkstra(temp_netCostMatrix, P_(index_dev_vertex), destination);
                if ~isempty(dev_p)
